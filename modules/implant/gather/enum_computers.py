...
class EnumComputersJob(core.job.Job):
    def create(self):
        self.fork32Bit = True
    ...
    def report(self, handler, data, sanitize = False):
        ...
        if data == "Complete":
            super(EnumComputersJob, self).report(handler, data)
        elif "***" in data:
            self.parse_computers_data(data)

        handler.reply(200)

    def parse_computers_data(self, data):
        self.print_good("Computer list retrieved")
        computers = data.split("***")
        for computer in computers:
            self.shell.print_plain(computer)
...
class EnumComputersImplant(core.implant.Implant):

    NAME = "Enum Computers"
    DESCRIPTION = "Enumerates domain computers using NetServerEnum"
    AUTHORS = ["TheNaterz"]
    ...
    def run(self):
        ...
        workloads = {}
        workloads["js"] = self.loader.load_script("data/implant/gather/enum_computers.js", self.options)

        self.dispatch(workloads, EnumComputersJob)
